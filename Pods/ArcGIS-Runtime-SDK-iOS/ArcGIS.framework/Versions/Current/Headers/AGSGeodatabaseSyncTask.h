...
/** Import changes from a delta file into the geodatabase. This is useful in workflows where the client does not directly connect to the originating service to synchronize changes with the server (either because the client does not have access to the service, or to improve scalability where it is impractical for a large number of clients to depend upon the service). Rather, the changes from the service are made available as a delta file to all clients which then import them into their copy of the geodatabase.
 @param geodatabase into which to import the delta
 @param inputPath of the delta file containing the changes
