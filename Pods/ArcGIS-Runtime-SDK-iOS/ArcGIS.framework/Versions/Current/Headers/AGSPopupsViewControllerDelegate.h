...
/** Tells the delegate that a sketch editor is required for an  AGSPopup. The popupsViewController will listen for changes to the  AGSSketchEditor to determine when the geometry is in a valid state. Depending upon the type of geometry (point, polygon, polyline, etc) required for the  id<AGSGeoElement>, the delegate must return a sketch editor for editing the appropriate type of geometry. At this point the sketchEditor should be started and assigned to the appropriate  AGSMapView.
 ...
 @param popup Popup of the  id<AGSGeoElement> for which a sketchEditor is required.
 @since 100
 */
...
/** Tells the delegate that a user is ready to edit the geometry for
 ...
 editing begins. 
 The sketchEditor is the sketchEditor that was earlier passed to the popupsViewController through the  popupsViewController:sketchEditorForPopup: delegate method.
 ...
 @param popup whose  id<AGSGeoElement>'s geometry needs to be edited
 @since 100
 */
...
/** The method called by the AGSPopupsViewController when the user begins editing the current popup.
 @param popupsViewController The popup view controller
 @param popup whose  id<AGSGeoElement> needs to be edited
 @since 100
 */
...
/** The method called by the AGSPopupsViewController when the user is done editing the current popup.
 @note This is where you might make the appropriate calls to keep the server in sync with the client. For example, you might call  applyEditsWithCompletion if dealing with  AGSArcGISFeature objects.
 @param popupsViewController The popup view controller
 @param popup whose  id<AGSGeoElement> was edited
 @since 100
 */
...
/** The method called by the AGSPopupsViewController when the user cancels editing the current popup.
 @param popupsViewController The popup view controller
 @param popup whose  id<AGSGeoElement> was started to be edited but later canceled
 @since 100
 */
-(void)popupsViewController:(AGSPopupsViewController *)popupsViewController didCancelEditingForPopup:(AGSPopup*)popup;

/** The method called by the AGSPopupsViewController when the user confirms they want to delete the  id<AGSGeoElement>.
 The implementer of this method must actually delete the  id<AGSGeoElement>.
 @note This method will only be called for popups with a  id<AGSGeoElement> that we don't know how to delete.
 Currently, the popup only knows how to delete  AGSArcGISFeature objects. This method will be called for all other types of  id<AGSGeoElement> objects.
 @param popupsViewController The popup view controller
 @param popup whose  id<AGSGeoElement> needs to be deleted
...
/** The method called by the AGSPopupsViewController when a  id<AGSGeoElement> was successfully deleted.
 @param popupsViewController The popup view controller
 @param popup whose  id<AGSGeoElement> was deleted
 @since 100
 */
-(void)popupsViewController:(AGSPopupsViewController *)popupsViewController didDeleteForPopup:(AGSPopup*)popup;

/** The method called by the AGSPopupsViewController when it fails to delete a  id<AGSGeoElement>.
 ...
 @param popupsViewController The popup view controller
 @param popup whose  id<AGSGeoElement> was meant to be deleted
...
/** The method called by the AGSPopupsViewController when a popup's validity is changed. This happens
 when the geometry or attributes of the popup's  id<AGSGeoElement> are changed. isValid will be YES if it is
 determined that a  id<AGSGeoElement> meets it's requirements to be saved to it's datasource.
 This method is useful if you have your own "finish editing" button and you want to enable/disable it
 when the  id<AGSGeoElement> is valid.
 @param popupsViewController The popup view controller
 @param isValid Whether or not the  id<AGSGeoElement> meets it's known requirements to be saved to it's datasource.
 @since 100
 */
...
/** The AGSPopupsViewController will call this method on the delegate when a view controller needs to be presented.
 ...
 @param rect The rect is in the coordinate space of the fromViewController. Only applicable if viewType is  AGSPopupViewControllerTypeUIImagePicker
 @since 100
 */
...
/** The AGSPopupsViewController will call this when the user switches a feature type for the current popup.
 Potentially at this point symbology will change for the feature. Currently this is only called for  AGSArcGISFeature popups.
 @since 100
 */
