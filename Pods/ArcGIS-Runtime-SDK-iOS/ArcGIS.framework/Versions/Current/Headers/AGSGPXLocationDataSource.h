...
/** Initializes the datasource using a URL to the GPX file (on the web or on disk)
 @note This method is synchronous, it will block the calling thread unil the file is
 loaded and parsed. Therefore, you should call it on a background thread
 to avoid locking up the main UI thread.
 
 @param gpxUrl URL to a GPX file on the web or on disk
 @since 100
 */
-(instancetype)initWithURL:(NSURL*)URL;

/** Initialize this object with the name of a GPX (.gpx file) within the application bundle or shared documents directory.
 @note This method is synchronous, it will block the calling thread unil the file is loaded and parsed. Therefore, you should call it on a background thread
 to avoid locking up the main UI thread.
...
/** Initialize the datasource using GPX data
 @note This method is synchronous, it will block the calling thread unil the data is parsed. Therefore, you should call it on a background thread
 to avoid locking up the main UI thread.
